...
class App extends AppWithD2 {
    ...
    componentDidMount() {
        ...
        if (appConfig && appConfig.feedback) $.feedbackDhis2(d2, appKey, appConfig.feedback);
        ...
        const nonAllSectionSelected$ = appState
            ...
            .map(state =>
                ...
                state.mainSections.some(
                    mainSection => mainSection.key === state.sideBar.currentSection
                )
            );
    ...
    render() {
        ...
        const showAppTitle = appConfig.get("appearance.header.showTitle")
            ? appConfig.get("appKey")
            : undefined;
