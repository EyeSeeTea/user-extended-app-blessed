...
function runValidatorFunctions(validatorFunctions, value) {
    return (
        Promise.all(validatorFunctions.map(validator => validator.call(null, value)))
    ...
    listenToValidatorsFor(fieldName) {
        return (
            this.validatorPipeline
                ...
                .map(field => {
                    return Observable.fromPromise(runValidatorFunctions(field.asyncValidators, field.value)).map(
                        status => {
