...
export default class BatchModelsMultiSelectModel {
    ...
    getParents(parents) {
        const parentIds = _(parents).map(obj => obj.id).compact().value();
        const options = {
            paging: false,
    ...
    getSelectedChildren(parents) {
        ...
        return _(commonChildren).sortBy(obj => obj.name).value();
    }
    ...
    getNewChildren(parents, newChildren, updateStrategy) {
        ...
        const getNewChildrenForParent = (parent) => {
            switch(updateStrategy) {
                case "merge":
                    const childrenToRemove = _.differenceBy(childrenAssignedToAllParents, newChildren, "id");
                    const childrenToAdd = _.differenceBy(newChildren, childrenAssignedToAllParents, "id");
